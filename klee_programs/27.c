...
int count_operands(const char* expression) {
    ...
    for (int i = 0; i < length; i++) {
        if (is_digit(expression[i]) || is_letter(expression[i])) {
            count++;
        }
    }
...
int main() {
    ...
    const char* simple_infix = "1+2";
    int success = infix_to_postfix(simple_infix, postfix);
    klee_assert(success);
    klee_assert(strcmp(postfix, "1 2 +") == 0);
    
    const char* simple_infix2 = "2*3";
    success = infix_to_postfix(simple_infix2, postfix);
    klee_assert(success);
    klee_assert(strcmp(postfix, "2 3 *") == 0);
    
    // Test expression with parentheses
    const char* paren_infix = "(1+2)*3";
    success = infix_to_postfix(paren_infix, postfix);
    klee_assert(success);
    klee_assert(strcmp(postfix, "1 2 + 3 *") == 0);
    ...
    // Test validation
    klee_assert(is_valid_expression("1+2"));
    klee_assert(is_valid_expression("(1+2)*3"));
    klee_assert(!is_valid_expression("(1+2"));
    klee_assert(!is_valid_expression("1+2)"));
    
    // Test counting
    klee_assert(count_operators("1+2*3") == 2);
    klee_assert(count_operands("1+2*3") == 3);
    ...
    const char* single_infix = "5";
    success = infix_to_postfix(single_infix, postfix);
    klee_assert(success);
    klee_assert(strcmp(postfix, "5") == 0);
